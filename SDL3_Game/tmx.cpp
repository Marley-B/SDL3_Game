            ...
            child != nullptr; child = child->NextSiblingElement())
        {
            if (strcmp(child->Name(), "tileset") == 0)
            {
                ...
                if (ts)
                {
                    ...
                    // Check if individual tiles are defined
                    ...
                    // If no individual tiles defined, use the tileset's main image
                    if (!hasIndividualTiles)
                    {
                        XMLElement* image = ts->FirstChildElement("image");
                        if (image)
                        {
                            // Create a single tile entry pointing to the main image
                            // This image will be used for all tiles in the set
                            Tile mainTile;
                            mainTile.id = 0;
                            mainTile.image.source = image->Attribute("source");
                            mainTile.image.width = image->IntAttribute("width");
                            mainTile.image.height = image->IntAttribute("height");
                            newTileSet.tiles.push_back(mainTile);
                        }
                    }
